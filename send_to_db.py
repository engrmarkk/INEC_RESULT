...
agents = [(1, 'Christian', 'Emenike', 'christian@bincom.net', '08034699500', 1),
...
lga_res = [(1, '33', 'PDP', 78129, '', '2011-04-27 09:46:05', '192.168.1.110'),
...
pu_res = [(111, '8', 'PDP', 802, 'Bose', '2011-04-26 15:44:03', '192.168.1.101'),
...
lgas = [(1, 1, 'Aniocha North', 25, 'Aniocha North', 'Bincom', '0000-00-00 00:00:00', '127.0.0.2'),
...
pus = [(8, 6, 8, 17, 181, 'DT1708006', 'Sapele Ward 8 PU _', 'NULL', '5.59371889', '5.999311165', 'NULL', 'NULL', 'NULL'),
...
wards = [(1, 2, 'Aba - Unor', 2, 'Aba - Unor', 'Bincom', '0000-00-00 00:00:00', '127.0.0.1'),
...
def send_db():
    for agent in agents:
        send_agent = AgentName(
            name_id=agent[0],
            firstname=agent[1],
            lastname=agent[2],
            email=agent[3],
            phone=agent[4],
            pollingunit_uniqueid=agent[5],
        )
        send_agent.save()

    for lga in lga_res:
        send_lga = AnnLgaResult(
            result_id=lga[0],
            lga_name=lga[1],
            party_abbreviation=lga[1],
            party_score=lga[3],
            entered_by_user=lga[4],
            date_entered=lga[5],
            user_ip_address=lga[6]
        )
        send_lga.save()
    for pu_ in pu_res:
        send_pures = AnnPuResult(
            result_id=pu_[0],
            polling_unit_uniqueid=pu_[1],
            party_abbreviation=pu_[2],
            party_score=pu_[3],
            entered_by_user=pu_[4],
            date_entered=pu_[5],
            user_ip_address=pu_[6]
        )
        send_pures.save()
    ...
    for pu in pus:
        send_pu = PollingUnit(
            ...
            lga_id=pu[3],
            uniquewardid=pu[4],
            polling_unit_number=pu[5],
            polling_unit_name=pu[6],
            polling_unit_description=pu[7],
            lat=pu[8],
            long=pu[9],
            entered_by_user=pu[10],
            date_entered=pu[11],
            user_ip_address=pu[12]
        )
        send_pu.save()
    for state in states:
        send_state = State(
